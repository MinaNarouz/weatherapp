...
btn.addEventListener('click', async function getAndProcessTemp() {
    let zipCode = document.getElementById("zip").value;
    let countryCode = document.getElementById("country").value;
    let feelLike = document.getElementById("feelings").value;
    const apiFullUrl = `https://api.openweathermap.org/data/2.5/weather?zip=${zipCode},${countryCode}&appid=${api_key}`;
  ...
  // Using try to make a post with the data
   try{
    await fetch("/saveWeatherData", {
      ...
      document.getElementById("temp").innerHTML = `<p>${reviewDataJson.temp} F</p>`
